...
# Importar desde agents si está disponible
try:
    import sys
    ...
    # Cargar .env desde architecture
    architecture_env_path = os.path.join(os.path.dirname(__file__), "..", ".env")
    load_dotenv(architecture_env_path)
...
def encode_frame_to_base64(frame: np.ndarray, frame_number: int = 0, 
    ...
    try:
        ...
        if save_to_file:
            try:
                # Buscar directorio architecture
                current_dir = os.path.dirname(os.path.abspath(__file__))
                architecture_path = os.path.join(current_dir, "..")
                
                if os.path.exists(architecture_path):
                    base64_filename = os.path.join(architecture_path, "frame_base64_1.txt")
                    with open(base64_filename, "w", encoding="utf-8") as f:
                        f.write(frame_base64)
                    print(f"💾 Base64 guardado en: {base64_filename}")
                else:
                    # Intentar guardar en el directorio actual si no se encuentra architecture
